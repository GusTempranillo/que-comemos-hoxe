...
- [ ] **Preparar unha migración aditiva desde o esquema JSON real antes de aplicar a Fase 3.** A revisión de 2026-08-06 confirmou que `qch_receitas`, `qch_ingredientes` e `qch_persoas` son hoxe `id + data jsonb`; `DATABASE_SCHEMA.sql`/`DATABASE_SEED.sql` non se poden executar directamente porque `create table if not exists` non transforma esas táboas. A migración deberá crear as relacións, extraer os datos do JSON, recompoñer o contrato nos workflows e verificarse de punta a punta. `semana` e `cociñeiros` mantéñense en `qch_estado` por agora: non conteñen datas coas que poboar `qch_planificacion` sen inventar información. Ver `BACKEND_N8N_STATUS.md` §"Fase 3: estado confirmado".
